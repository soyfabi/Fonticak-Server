                           ...
                           uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count)
{
	...
	if (Creature* movingCreature = thing->getCreature()) {
		...
		if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(), player->getPosition())) {
			//Push in Creature //
			uint32_t pushDelayOfShort = g_config.getNumber(ConfigManager::PUSH_SHORT);
			SchedulerTask* task = createSchedulerTask(pushDelayOfShort, std::bind(&Game::playerMoveCreatureByID, this, player->getID(),
...
void Game::playerMoveCreature(Player* player, Creature* movingCreature, const Position& movingCreatureOrigPos, Tile* toTile)
{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos, player->getPosition())) {
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			//Push in Creature / Player//
			uint32_t pushDelayInCreatureDistance = g_config.getNumber(ConfigManager::PUSH_DISTANCE);				
			SchedulerTask* task = createSchedulerTask(pushDelayInCreatureDistance, std::bind(&Game::playerMoveCreatureByID, this,
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos)) {
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
			g_dispatcher.addTask(createTask(std::bind(&Game::playerAutoWalk,
			                                this, player->getID(), std::move(listDir))));
			//Push in Item//
			uint32_t pushDelayInItem = g_config.getNumber(ConfigManager::PUSH_ITEM);	
			SchedulerTask* task = createSchedulerTask(pushDelayInItem, std::bind(&Game::playerMoveItemByPlayerID, this,
			                      player->getID(), fromPos, spriteId, fromStackPos, toPos, count));
			player->setNextWalkActionTask(task);
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
...
void Game::internalDecayItem(Item* item)
{
	...
	if (it.decayTo != 0) {
		...
		if (player && player->isEquippedItem(item)) {
