	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE) {
			...
			if (attackSpeed) {
				...
				s << "Atk Speed:" << (attackSpeed / 1000.) << "s";
			}
		...
		if (it.abilities) {
            ...
            if (it.abilities->healthGain) {
				...
				s << "Health +" << it.abilities->healthGain << " each/" << std::fixed << std::setprecision(1) << it.abilities->healthTicks / 1000. << "s";
            }
            ...
            if (it.abilities->manaGain) {
                ...
                s << "Mana +" << it.abilities->manaGain << " each/" << std::fixed << std::setprecision(1) << it.abilities->manaTicks / 1000. << "s";
            }
		}
