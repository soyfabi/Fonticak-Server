...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(RETURNVALUE_SUPPLYSTASHISFULL)
	...
	registerEnumIn("configKeys", ConfigManager::ALLOW_RELOAD);
	registerEnumIn("configKeys", ConfigManager::PUSH_CRUZADO);
	registerEnumIn("configKeys", ConfigManager::GUILD_SQUARE);
	registerEnumIn("configKeys", ConfigManager::SHOW_EXIVA_MONSTER);
	registerEnumIn("configKeys", ConfigManager::REGENERATION_PZ);
	...
	registerEnumIn("configKeys", ConfigManager::PUSH_ITEM);
	registerEnumIn("configKeys", ConfigManager::PUSH_SHORT);
	registerEnumIn("configKeys", ConfigManager::PUSH_DISTANCE);
...
int LuaScriptInterface::luaLoadOutfits(lua_State* L)
{
	//loadOutfits(outfits)
	if (!isTable(L, -1)) {
		reportErrorFunc(L, "[LuaLoadOutfits::ERROR] Outfits table missing!");
		pushBoolean(L, false);
		return 1;
	}
	...
	if (len == 0) {
		reportErrorFunc(L,  "[LuaLoadOutfits::ERROR] Cannot find outfits");
		pushBoolean(L, false);
		return 1;
	}
...
int LuaScriptInterface::luaGuildAddExperience(lua_State* L)
{
	...
	Guild* guild = getUserdata<Guild>(L, 1);
	if (guild) {
		uint64_t points = getNumber<int64_t>(L, 2);
		guild->addExperience(points);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
