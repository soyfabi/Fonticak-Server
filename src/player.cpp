...
Player::Player(ProtocolGame_ptr p) :
	Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), supplystash(new SupplyStash(ITEM_SUPPLY_STASH)), client(std::move(p))
{
...
Player::~Player()
{
	...
	if (depotLocker) {
		...
		depotLocker->removeSupplyStash(supplystash);
	}
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	s << "\nThe level of that Guild is " << guild->getLevel() << ".";
	return s.str();
}
