...
bool ConfigManager::load()
{
	...
	boolean[LOAD_OUTFITS_FROM_LUA] = getGlobalBoolean(L, "loadOutfitsFromLua", false);
	boolean[ALLOW_RELOAD] = getGlobalBoolean(L, "allowReload", false);
	boolean[PUSH_CRUZADO] = getGlobalBoolean(L, "pushCruzado", false);
	boolean[GUILD_SQUARE] = getGlobalBoolean(L, "guildLeaderSquare", false);
	boolean[REGENERATION_PZ] = getGlobalBoolean(L, "regenerationPZ", false);
	boolean[SHOW_EXIVA_MONSTER] = getGlobalBoolean(L, "showExivaMonster", false);
	...
	integer[PUSH_ITEM] = getGlobalNumber(L, "pushDelayInItem", 1000);
	integer[PUSH_SHORT] = getGlobalNumber(L, "pushDelayOfShort", 1000);
	integer[PUSH_DISTANCE] = getGlobalNumber(L, "pushDelayInCreatureDistance", 1000);
