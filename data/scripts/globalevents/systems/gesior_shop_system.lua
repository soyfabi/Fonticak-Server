				...
				if(not item_doesnt_exist) then
					...
					local receipt = Game.createItem(24301)
						if(add_item_type == 'container') then
							...
							received_item = inbox:addItemEx(new_container, INDEX_WHEREEVER, FLAG_NOLIMIT)	
						else					
							received_item = inbox:addItem(param1, param2, INDEX_WHEREEVER, FLAG_NOLIMIT)
					
							inbox:addItemEx(letter, INDEX_WHEREEVER, FLAG_NOLIMIT)
							letter:setAttribute(ITEM_ATTRIBUTE_TEXT, "             >> RECEIPT   OF   SALE <<\n-----------------------------\nPackage delivered at: " .. getFormattedWorldTime() .."pm\nBy: ".. player:getName() ..".\nFrom: www.fonticak-server.com\n-----------------------------\n                     ITEM             AMT\n-----------------------------\n".. add_item_name .."")
							letter:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "From: ".. player:getName() ..".")
						end

						if(received_item) then
							player:sendTextMessage(messageType, "The package has just arrived: \n>> ".. add_item_name .." <<\nIt will be delivered to the Inbox inside the Depot Locker.")
...
shopSystemGlobalEvent:interval(5000)
