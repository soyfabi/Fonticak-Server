...
local event = CreatureEvent("Exiva_Kill")
...
function event.onKill(creature, target)
    ...
    exiva_fuc[playerId] = {target:getName(), os.time()}
...
local login = CreatureEvent("Exiva_Login")

function login.onLogin(player)
    player:registerEvent("Exiva_Kill")
    return true
end
...
local exiva_config = configManager.getBoolean(configKeys.SHOW_EXIVA_MONSTER)
local LEVEL_LOWER = 1
local LEVEL_SAME = 2
...
function spell.onCastSpell(creature, variant)
	...
	local exiv = exiva_fuc[targetId]
	if exiv and exiva_config then
		message = message .. ".\nLast target: ".. exiva_fuc[targetId][1] .." it was " .. string.diff((os.time() - exiva_fuc[targetId][2]), true) .. " ago"
	end
