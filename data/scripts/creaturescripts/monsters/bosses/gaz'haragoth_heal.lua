local creatureevent = CreatureEvent("GazharagothHeal")
...
function creatureevent.onThink(creature)
local hp = (creature:getHealth()/creature:getMaxHealth())*100
	if (hp < 12.5 and not creature:getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT, 88888)) then
		creature:addCondition(condition)
		creature:say("Gaz'haragoth begins to draw on the nightmares to HEAL himself!")
		addEvent(function(cid)
			...
			creature:say("Gaz'haragoth HEALS himself!")
			return true
